...
1. The report: a rendered page, a URL, or its source (`.qmd`, `.Rmd`, `.md`, HTML). Required.
2. The previous edition. Optional; without it OR-05, OR-16 and OR-17 are checked only as far as the current page allows.
3. Methods documentation or a code repository, where the report links to one. Optional; without it OR-24 and OR-25 are checked only as far as the report itself allows.
...
2. Read the report. Identify the headline quantities, whether the report forecasts (OR-19), whether it expresses probability in words (OR-08), and whether it rests on data subject to reporting delay (OR-14).
...
- Where the items name a published scale (CDC, WHO, IPCC or GRADE confidence at OR-09; PHIA or IPCC probability at OR-08), offer it as one choice among others, not as required.
...
- Keep uncertainty in the estimate (OR-06 to OR-08) apart from confidence in the evidence (OR-09). Never offer a confidence statement that only restates the interval, or a probability word as a confidence level.
- An alternative that changes presentation costs continuity (OR-17). Say so, and suggest announcing the change (OR-16).
